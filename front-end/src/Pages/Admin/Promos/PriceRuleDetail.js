...
const PriceRuleDetail = () => {
  ...
  return (
    <div className="container mt-7 mb-5">
      ...
      <p>
        <strong>Title:</strong> {priceRule.title}
      </p>
      <p>
        <strong>Value Type:</strong>{' '}
        {new Date(priceRule.value_type).toLocaleString()}
      </p>
      <p>
        <strong>Value:</strong> {priceRule.value}
      </p>
      <p>
        <strong>Starts At:</strong>{' '}
        {new Date(priceRule.starts_at).toLocaleString()}
      </p>
      <p>
        <strong>Ends At:</strong>{' '}
        {priceRule.ends_at
          ? new Date(priceRule.ends_at).toLocaleString()
          : 'N/A'}
      </p>
      <p>
        <strong>Allocation Method:</strong>{' '}
        {priceRule.allocation_method || 'N/A'}
      </p>
      <p>
        <strong>Target Selection:</strong> {priceRule.target_selection || 'N/A'}
      </p>
      <p>
        <strong>Target Type:</strong> {priceRule.target_type || 'N/A'}
      </p>
      <p>
        <strong>Usage Limit:</strong> {priceRule.usage_limit || 'Unlimited'}
      </p>
      <p>
        <strong>Prerequisite Subtotal:</strong>{' '}
        {priceRule.prerequisite_subtotal_price || 'None'}
      </p>
      <p>
        <strong>Prerequisite Quantity:</strong>{' '}
        {priceRule.prerequisite_quantity_range || 'None'}
      </p>
      <p>
        <strong>Created At:</strong>{' '}
        {new Date(priceRule.created_at).toLocaleString()}
      </p>
      <p>
        <strong>Updated At:</strong>{' '}
        {new Date(priceRule.updated_at).toLocaleString()}
      </p>
      ...
      <div>
        ...
        <button
          className="btn btn-primary mt-3"
          onClick={() => handleDiscountCreate(formData.code)}
        >
          Submit
        </button>
      </div>
