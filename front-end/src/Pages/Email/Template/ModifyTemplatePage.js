...
const EditTemplatePage = () => {
    ...
    useEffect(() => {
        ...
        const fetchTemplate = async () => {
            try {
                setIsLoading(true);
                const response = await axios.get(`${environment}/gateway/api/ProxyTemplate/${templatename}`, {
                    headers: {
                        accept: '*/*',
                    },
                });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setIsLoading(true);
            const response = await axios.put(`${environment}/gateway/api/ProxyTemplate/${templatename}`, payload, {
                headers: {
                    accept: '*/*',
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            {submitSuccess !== null && (
                                <div className={`mb-3 alert ${submitSuccess ? 'alert-success' : 'alert-danger'}`}>
                                    {submitSuccess ? 'Template updated successfully!' : 'Failed to update the template.'}
                                </div>
                            )}
